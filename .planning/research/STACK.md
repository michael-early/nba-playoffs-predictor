...
**Confidence:** MEDIUM — all library and version claims are from training data (cutoff Aug 2025). External tools were unavailable; verify versions with `pip index versions <package>` before pinning.
...
# Core data + ML
pip install pandas==2.2.* numpy==1.26.* scikit-learn==1.5.* statsmodels==0.14.* scipy==1.13.*

# Sport-specific data
pip install nfl-data-py nba_api pybaseball

# Visualization
pip install matplotlib==3.9.* seaborn==0.13.* plotly
...
# Dev / tooling
pip install ruff pytest tqdm python-dotenv nbstripout pre-commit
```
...
**For EDA notebooks (NFL combine analysis):**
...
**For repeatable model scripts (scripts/ folder):**
...
**For tree-based models (later phase):**
...
**If dataset grows large (Statcast play-by-play, millions of rows):**
...
---
*Stack research for: Multi-sport ML research repository (NFL, NBA, MLB)*
*Researched: 2026-04-29*
